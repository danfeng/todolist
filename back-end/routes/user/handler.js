...
exports.getUserInfo = (req, res) => {
    ...
    const mysqlStr = 'select id,username,phone,name,userPic as photo from ev_users where id=?'
    db.query(mysqlStr, req.auth.id, (err, results) => {
...
exports.getUserList = function (req, res) {
    ...
    *
    ...
    db.query(sqlStr, (err, results) => {
        console.log(err)
        if (err) return res.cc(500, '数据库查询错误1')
...
exports.getOneRole = function (req, res) {
    ...
    const sqlStr = `select * from ev_role where roleId=?`
    // 执行查询
    db.query(sqlStr, roleId, (err, results) => {
        if (err) return res.cc(500, '数据库查询错误')
        if (results.length !== 1) return res.cc('角色信息不存在')
...
exports.addRole = function (req, res) {
    ...
    db.query(sqlStr, { roleName }, (err, results) => {
        if (err) return res.cc(500, '数据库查询错误')
